...
class RecallRelay:
    ...
    def __init__(self, recall_url: str, *, recall_key: str | None = None, gemini_key: str | None = None, openai_key: str | None = None, workspace_id: str | None = None, workspace_key: str | None = None, timeout: float = 180.0) -> None:
        ...
        if not recall_url.startswith(("http://", "https://")): raise ValueError("recall_url must be an absolute HTTP(S) URL")
    ...
    def _post(self, url: str, body: dict[str, Any], headers: dict[str, str] | None = None) -> dict[str, Any]:
        ...
        request=Request(url, data=json.dumps(body).encode(), headers={"Content-Type":"application/json", "User-Agent":"recall-relay/0.1", **workspace_headers, **(headers or {})})
        try:
            return json.load(urlopen(request, timeout=self.timeout))
    ...
    def generate_with(self, prompt: str, generator: Callable[[str], bytes], *, provider: str, model: str, media_type: str="image/png", tags: list[str] | None=None, intent: dict[str, str] | None=None, params: dict[str, Any] | None=None, cost_usd: float | None=None) -> RelayResult:
        ...
        if not media_type.strip() or "/" not in media_type: raise ValueError("media_type must be a MIME type")
        ...
        captured = self._capture(prompt=prompt, media_base64=encoded, media_type=media_type, provider=provider, model=model, tags=tags, intent=intent, params={"relay":"custom-provider", **params}, cost_usd=cost_usd)
